...
1. 下載並解壓縮發佈包，會拿到兩個東西：
   ...
   - `usagi-shimeji-v3.zip` — 兔兔素材包（**不要解壓縮**）
...
3. 把 `usagi-shimeji-v3.zip` **直接拖進** Shijima 的視窗完成匯入。
...
| 🍅 番茄鐘 | 每工作 25 分鐘，全部兔兔變成約半個螢幕高的巨兔提醒你休息，5 分鐘後自動縮回重新計時；右鍵可「⏭ 跳過休息」或整個關掉（取代舊的 50 分久坐通知，關掉後久坐通知會回來） |
...
- **沒聽到叫聲？** 檢查右鍵選單「🔊 兔兔叫聲」有勾；叫聲跟吶喊泡泡一起出現（十幾分鐘一次，好感度越高越常喊），想立刻聽就右鍵 → 🎉 下班狂奔。從 v1.1 升級的人要重新匯入 usagi-shimeji-v3.zip 才有聲音檔。
