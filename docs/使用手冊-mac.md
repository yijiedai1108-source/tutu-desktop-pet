...
- **沒聽到叫聲？** 檢查右鍵選單「🔊 兔兔叫聲」有勾；叫聲跟吶喊泡泡一起出現（十幾分鐘一次，好感度越高越常喊），想立刻聽就右鍵 → 🎉 下班狂奔。**v1.4 以前匯入過的人一定要重新匯入 v4 素材包**（舊包有匯入 bug：聲音檔不會被解出、清單名字會變成「img」）。
- **清單裡有個叫「img」的？** 那是舊素材包的匯入 bug 產物：在管理視窗把它選起來刪掉，重新匯入 `usagi-shimeji-v4.zip` 後改用 **Usagi**。
