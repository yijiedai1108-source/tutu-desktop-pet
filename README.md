...
1. 解壓縮發佈包，會拿到兩個東西：
   ...
   - `usagi-shimeji-v4.zip` — 兔兔素材包（**不要解壓縮**）
...
3. 把 `usagi-shimeji-v4.zip` **直接拖進** Shijima 的視窗完成匯入。
...
| 🍅 番茄鐘 | 每工作 25 分鐘，全部兔兔變成約 1/3 螢幕高的巨兔（高清素材，不會糊）提醒你休息，5 分鐘後自動縮回重新計時；右鍵可「⏭ 跳過休息」或整個關掉 |
| 🧘 久坐提醒 | 每 50 分鐘通知＋兔兔做伸展操（番茄鐘開啟時自動靜音，關掉番茄鐘才會出現） |
...
- **沒聽到叫聲？** 檢查右鍵選單「🔊 兔兔叫聲」有勾；叫聲跟吶喊泡泡一起出現（十幾分鐘一次，好感度越高越常喊），想立刻聽就右鍵 → 🎉 下班狂奔。**v1.5 以前匯入過的人一定要重新匯入 v4 素材包**（舊包有匯入 bug：聲音檔不會被解出）。
- **清單裡有個叫「img」的？／兔兔放著會定住不動？** 都是舊素材包匯入 bug 的症狀：在管理視窗刪掉「img」，重新匯入 `usagi-shimeji-v4.zip`，改用 **Usagi** 就都好了。
